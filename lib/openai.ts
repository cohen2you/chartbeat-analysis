...
Please format your response as JSON with this structure:
{
  ...
  "statsByAuthor": "A comprehensive statistical breakdown section that LISTS EVERY SINGLE AUTHOR from the CSV file with their complete statistics. CRITICAL: Do NOT summarize or provide examples - you MUST provide a complete detailed list of ALL authors. For EACH and EVERY author, provide ALL of the following in a clear, structured format: Author name as header, total articles published (exact number), total page views (exact number and % of overall), total unique visitors (exact number and % of overall) - ONLY if that column exists, total quality views (exact number and % of overall) - ONLY if that column exists, average views per article, average uniques per article - ONLY if that column exists, average quality view rate (%) - ONLY if that column exists, average time on page (seconds) - ONLY if that column exists, best performing article (FULL title, exact views, exact uniques if available, exact quality views if available, quality rate % if available, avg time if available), worst performing article (FULL title, exact views, exact uniques if available, exact quality views if available, quality rate % if available, avg time if available), performance consistency (articles above/below average with exact numbers and percentages), top 3-5 sections (with exact view counts and %) - ONLY if sections column exists, top 3-5 referrers (with exact view counts and %) - ONLY if referrers column exists, date range (earliest and latest publish dates), and notable trends. Format each author as a separate section with clear headers. Include EVERY SINGLE author - do not skip any. Use as many tokens as needed (up to the limit). IMPORTANT: This must be a STRING, not an object. Format as plain text with clear section separators for each author. ONLY include fields that exist in the data."
}`;
...
Please format your response as JSON with this structure:
{
  ...
  "statsByAuthor": "A comprehensive statistical breakdown section that LISTS EVERY SINGLE AUTHOR across ALL datasets with their complete statistics. CRITICAL: Do NOT summarize or provide examples - you MUST provide a complete detailed list of ALL authors. For EACH and EVERY author, provide ALL of the following in a clear, structured format: Author name as header, total articles published across all datasets (exact number), total page views (exact number and % of overall), total unique visitors (exact number and % of overall) - ONLY if that column exists, total quality views (exact number and % of overall) - ONLY if that column exists, average views per article, average uniques per article - ONLY if that column exists, average quality view rate (%) - ONLY if that column exists, average time on page (seconds) - ONLY if that column exists, best performing article across all datasets (FULL title, exact views, exact uniques if available, exact quality views if available, quality rate % if available, avg time if available, dataset name), worst performing article across all datasets (FULL title, exact views, exact uniques if available, exact quality views if available, quality rate % if available, avg time if available, dataset name), performance comparison between datasets if applicable (show numbers for each dataset), performance consistency (articles above/below average with exact numbers and percentages), top 3-5 sections (with exact view counts and %) - ONLY if sections column exists, top 3-5 referrers (with exact view counts and %) - ONLY if referrers column exists, date range (earliest and latest publish dates across all datasets), and notable trends across datasets. Format each author as a separate section with clear headers. Include EVERY SINGLE author - do not skip any. Use as many tokens as needed (up to the limit). IMPORTANT: This must be a STRING, not an object. Format as plain text with clear section separators for each author. ONLY include fields that exist in the data."
}`;
